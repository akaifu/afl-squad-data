...
def normName(name):
    # AFL Tables' game-by-game roster pages render names as "Lastname,
    # Firstname" (e.g. "Ah Chee, Callum") while every other source we use
    # (club sites, Draft Guru) uses "Firstname Lastname". Without
    # reordering this, AFL Tables data would never match anything else by
    # name, which silently breaks the entire age pipeline - it looks like
    # "no AFL Tables data" when really it's "data we failed to recognize
    # as the same player". Detect the comma and flip the order first.
    name = name.strip()
    if ',' in name:
